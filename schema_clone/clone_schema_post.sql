CREATE OR REPLACE FUNCTION clone_post_data_ddl(src_fdw_server TEXT, 
                                              remote_user TEXT, 
                                              db_snapshot_id TEXT,  
                                              tgt_fdw_server TEXT,
                                              tgt_user   TEXT,
...
BEGIN
   ...
   IF error_count > 0 THEN
     ...
     RAISE EXCEPTION 'failed to clone copy % schema. For more detail please see log file %/%_post.error',
   ...
   psql_command := format('PGUSER="%s" PGPASSWORD="%s" %s -X -q --pset pager=off -v ON_ERROR_ROLLBACK=on --single-transaction -f %s/%s.dmp  "%s" 2>%s/%s_psql.error',
   ...
   IF error_count > 0 THEN
     RAISE NOTICE 'error occurred during post ddl restore stage';
