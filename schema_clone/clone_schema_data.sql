...
BEGIN
   ...
   psql_sql_src  := format('PGUSER=%s PGPASSWORD=%s %s -X -q -v ON_ERROR_STOP=1 --pset pager=off "%s" -c ',src_user_name,src_passwd,psql_command,src_conn_info);
   psql_sql_tgt  := format('PGUSER=%s PGPASSWORD=%s %s -X -q -v ON_ERROR_STOP=1 --pset pager=off "%s" -c ',tgt_user_name,tgt_passwd,psql_command,tgt_conn_info);
